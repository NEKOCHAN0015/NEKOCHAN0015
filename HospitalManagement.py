while (True):
    ...
    mysql = mysql.connector.connect(host="localhost", user="root", passwd="Priyawati@20")
    mycursor = mysql.cursor()
    mycursor.execute("create database if not exists orchid_hospital")
    mycursor.execute("use city_hospitals")
    # creating the tables we need
    mycursor.execute("create table if not exists patient_detail(name varchar(30) primary key,sex varchar(15),age int(3),disease varchar(20),address varchar(50),contact varchar(15))")
                        ...
                        # entering the patient details table
                            ...
                            # adding new patient
                                ...
                                mycursor.execute("insert into patient_detail values('" + name + "','" + sex + "','" +
                                        age + "','" + disease + "','" + address + "','" + contact + "')")
                                mysql.commit()
                                mycursor.execute("select * from patient_detail")
                                for i in mycursor:
                                    v = list(i)
                                    k = ['NAME', 'SEX', 'AGE','DISEASE', 'ADDRESS', 'CONTACT']
                                    print(dict(zip(k, v)))
                                    print("""
                                        ====================================
